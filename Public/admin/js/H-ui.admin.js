...
function layer_show1(w,h,title,url){
	if (w == null || w == '') {
		w=800;
	};
	if (h == null || h == '') {
...
function sublist(w,h,txt,url){
	layer_show1(w,h,txt,url);
}

function goods_upload(w,h,txt,num){
	layer_show1(w,h,txt,'upload/type/goods/num/'+num+".html");
}

function upload(w,h,txt,type,formname){
	layer_show1(w,h,txt,'upload/type/'+type+'/formname/'+formname+".html");
}
/*左侧菜单响应式*/
	...
	/*function Uploadfile(){		
		if($("#uploads").length>0){ 
		  $('#uploads').on('click', function(){			
			  layer_show1(400,200,'上传图片','upload');
		  });
		}
	}*/
