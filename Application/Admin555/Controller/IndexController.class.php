...
//			$adbrands->delete($adbrands_data);
	...
	public function links(){
		...
		foreach($linkslist as $key => $val){
			$links_end_time = $val['links_end_time'];
			if($links_end_time < time()){
				$linkslist[$key]['is_show'] = 2;
			}
		}
		$this->assign('links_list',$linkslist);
		$this->assign('pagefooter',$this->showpage($nowPage,$this->getpagenum($count,C(ADMIN_DEFAULT_PAGENUM)),array()));
