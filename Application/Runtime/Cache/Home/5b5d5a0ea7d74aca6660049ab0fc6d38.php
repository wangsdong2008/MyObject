...
<title>购物车-<?php echo ($sys_sitename); ?></title>
...
<script src="<?php echo ($js_model_path); ?>/js/jquery.js"></script>
  ...
  <div class="container m10" style="background-image: none;">
    <div class="con_left" style="width: 100%">
      <!--  左侧正文内容部分  -->
      <div class="position">当前位置：<a href="<?php echo ($sys_url); ?>"><?php echo ($sys_sitename); ?></a>>  购物车</div>
      <div class="techlist border">
        <form action="<?php echo U('Index/order_info');?>" method="post" id="theForm">
        <table style="width: 100%" id="flowlist">
          <?php if(is_array($flowlist)): $i = 0; $__LIST__ = $flowlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$flow1list): $mod = ($i % 2 );++$i;?><tr>
            <td style="width: 3%;"><input type="checkbox" value="<?php echo ($flow1list["flow_id"]); ?>" name="flow_id[]"> </td>
            <?php if($flow1list["root_id"] == 29): ?><td><a href="<?php echo U('showcode',array('id'=>$flow1list['goods_id']));?>" target="_blank" title="<?php echo ($flow1list["goods_name"]); ?>"><?php echo ($flow1list["goods_name"]); ?></a></td>
              <?php else: ?>
              <td><a href="<?php echo U('showsoft',array('id'=>$flow1list['goods_id']));?>" target="_blank" title="<?php echo ($flow1list["goods_name"]); ?>"><?php echo ($flow1list["goods_name"]); ?></a></td><?php endif; ?>
          ...
          <tr>
            <td style="border-top: 1px solid #ccc; width: 100%; position: relative;" colspan="2"><span style="color: #333;font-size: 12px; }"><input type="checkbox" value="" id="allselect" name="allselect">全选</span>
              ...
              <label class="total" style="float: right; margin-right: 10px;">
                商品合计:<em id="lblTotal"><?php echo ($count); ?></em> 积分
              </label>
        ...
        <script type="text/javascript">
          $(function(){
            ...
            $("#delall").click(function(){
              ...
              if(j>=0){
                $("#theForm").attr("action","<?php echo U('delflowAll');?>");
                $("#theForm").submit();
              }else{
                alert('请选择您要删除的产品');
              }
