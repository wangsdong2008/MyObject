...
<title><?php echo ($GoodsDetail["goods_title"]); ?>-Ajax源码-源码下载-<?php echo ($sys_sitename); ?></title>
<meta name="Keywords" content="<?php echo ($GoodsDetail["goods_keyword"]); ?>">
<meta name="Description" content="<?php echo ($GoodsDetail["goods_description"]); ?>">
...
<script src="<?php echo ($js_model_path); ?>/js/jquery.js"></script>
    <script>
        $(function(){
            $("#kanbuq").click(function(){
                $("#passcode").attr("src","<?php echo U('verify');?>?t="+Math.random());
            });
        })
    </script>
  <!--  正文  -->
  <div class="container m10">
    <div class="con_left">
      <!--  左侧正文内容部分  -->
      <div class="position">当前位置：<a href="<?php echo ($sys_url); ?>"><?php echo ($sys_sitename); ?></a>><a href="<?php echo U('code');?>">源码下载</a>><a href="<?php echo U('codelist',array('id'=>$GoodsDetail['cat_id']));?>"><?php echo ($GoodsDetail["cat_name"]); ?></a>>&nbsp;&nbsp;正文</div>
          ...
          <div class="software-summary">
                ...
                <div style="">
                    ...
                    <ul class="clearfix software-infolist">
                        <li>
                            <strong>源码作者:</strong>
                            <span class="author"><?php echo ($GoodsDetail["true_name"]); ?></span>
                        </li>
                        <li class="right"><strong>源码大小:</strong> </li>
                        <li><strong>源码语言:</strong> 简体中文 </li>
                        <li class="right"><strong>运行环境:</strong><?php echo ($GoodsDetail["goods_environment"]); ?></li>
                        <li><strong>源码评级:</strong> <img src="<?php echo ($img_model_path); ?>/images/5star.gif" alt="5星级" /></li>
                        <li class="right"><strong>更新时间:</strong><?php echo (date("Y-m-d H:i:s",$GoodsDetail["goods_time"])); ?></li>
                        <li><strong>相关链接:</strong> Home Page </li>
                        <li class="right"><strong>演示地址:</strong> Demo Url </li>
          ...
          <div class="lastnext" style="margin-top: 0px;">
              <?php if(is_array($goods_about)): $k = 0; $__LIST__ = $goods_about;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods_about): $mod = ($k % 2 );++$k;?><p><?php if($k == 1): ?>上一篇<?php else: ?>下一篇<?php endif; ?>：
                      <?php if($goods_about["goods_id"] == 0): echo ($goods_about["goods_name"]); ?>
                          <?php else: ?>
                          <a href="<?php echo U('showcode',array('id'=>$goods_about['goods_id']));?>" title="<?php echo ($goods_about["goods_name"]); ?>"><?php echo ($goods_about["goods_name"]); ?></a><?php endif; ?>
  ...
  <div class="r_link border">
    ...
    <ul class="hourul">
        <?php if(is_array($hotgoodslist)): $i = 0; $__LIST__ = $hotgoodslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hotgoodslist): $mod = ($i % 2 );++$i;?><li class="onlink"><a href="<?php echo U('showcode',array('id'=>$hotgoodslist['goods_id']));?>" target="_blank" title="<?php echo ($hotgoodslist["goods_name"]); ?>"><?php echo ($hotgoodslist["goods_name"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
    </ul>
  </div>
  <div class="black"></div>
  <div class="r_link border">
    ...
    <ul class="hourul">
      <li class="onlink"><a href="http://www.aspbc.com/code/showcode.asp?id=207" target="_blank" title="easyui+asp Demo 演示程序">easyui+asp&nbsp;Demo&nbsp;演示程序</a></li>
